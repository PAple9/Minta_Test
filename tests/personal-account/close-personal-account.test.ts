import {
  ...
  CloseAccountCommand,
  AccountError,
  Account, AccountManagerImp, NotificationService, OpenAccountCommand, accountStorage
} from "../../source/personal-account";

describe('Personal-Account: CloseAccount', () => {
  const notificationService: jest.Mocked<NotificationService> = {sendWelcomeMessage: jest.fn()}
  const manager: AccountManager = new AccountManagerImp(notificationService);

  const command = new OpenAccountCommand();
  command.email = "customer0919@domain.ru";
  command.name = "Пугачева Ольга Сергеевна";
  command.id = "123-456-789";

  manager.openAccount(command);
